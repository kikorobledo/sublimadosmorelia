<div class="">
    ...
    <div class="mb-5">
        ...
        <h1 class="titulo-seccion text-3xl font-thin text-gray-500 mb-3">Banners</h1>
        ...
        <div class="flex justify-between">
            ...
            <button wire:click="openModalCreate" class="bg-gray-500 hover:shadow-lg hover:bg-gray-700 float-right mb-5 text-sm py-2 px-4 text-white rounded-full focus:outline-none hidden md:block">Agregar nuevo Banner</button>
                    ...
                    @foreach($banners as $banner)
                        ...
                        <tr class="text-sm font-medium text-gray-500 bg-white flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                            ...
                            <td class="px-3 py-3 w-full lg:w-auto p-3 text-gray-800 text-center lg:text-left lg:border-0 border border-b block lg:table-cell relative lg:static">
                                ...
                                <span class="lg:hidden absolute top-0 left-0 bg-blue-300 px-2 py-1 text-xs text-white font-bold uppercase rounded-br-xl">Imagenes</span>

                                <div class="flex space-x-3">

                                    @foreach ($banner->images as $image)

                                        <a href="{{ Storage::disk('images')->url($image->url) }}" data-lightbox="{{ $banner->id }}" data-title="Banenr image">
                                            <img class="w-10 lg:w-20 rounded" src="{{ Storage::disk('images')->url($image->url) }}" alt="Imagen">
                                        </a>
                            ...
                            <td class="px-3 py-3 w-full lg:w-auto p-3 text-gray-800 text-center lg:text-left lg:border-0 border border-b block lg:table-cell relative lg:static">
                                ...
                                @if($banner->created_by != null)

                                    <span class="font-semibold">Registrado por: {{$banner->createdBy->name}}</span> <br>
                            ...
                            <td class="px-3 py-3 w-full lg:w-auto p-3 text-gray-800 text-center lg:text-left lg:border-0 border border-b block lg:table-cell relative lg:static">
                                ...
                                @if($banner->updated_by != null)

                                    <span class="font-semibold">Actualizado por: {{$banner->updatedBy->name}}</span> <br>
                            ...
                            <td class="px-3 py-3 w-full lg:w-auto p-3 text-gray-800 text-center lg:text-left lg:border-0 border border-b lg:table-cell relative lg:static">
                                ...
                                <div class="flex justify-center lg:justify-start">
                                    ...
                                    <button
                                        wire:click="openModalEdit({{$banner}})"
                                        wire:loading.attr="disabled"
                                        wire:target="openModalEdit({{$banner}})"
                                    ...
                                    <button
                                        wire:click="openModalDelete({{$banner}})"
                                        wire:loading.attr="disabled"
                                        wire:target="openModalDelete({{$banner}})"
                    ...
                    <div>
                        ...
                        @foreach ($files_edit as $file)
                            ...
                            <div class="flex space-x-2 items-center mb-2">
                                    ...
                                    <img class="w-10 lg:w-20 rounded" src="{{ Storage::disk('images')->url($file['url']) }}" alt="Imagen">

                                <button
                                    wire:click="deleteImage({{$file['id']}})"
                                    wire:loading.attr="disabled"
                                    wire:target="deleteImage({{$file['id']}})"
                                    class="bg-red-400 hover:shadow-lg text-white text-xs md:text-sm px-3 py-2 rounded-full hover:bg-red-700 flex focus:outline-none"
                                >
    ...
    <x-jet-confirmation-modal wire:model="modalDelete">
        ...
        <x-slot name="content">
            ¿Esta seguro que desea eliminar el banner? No sera posible recuperar la información.
        </x-slot>
